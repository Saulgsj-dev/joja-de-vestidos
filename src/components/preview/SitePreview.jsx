...
export default function SitePreview({ config, sections, selectedSection }) {
  ...
  const renderSection = (section) => {
    switch (section.section_type) {
      case 'hero':
        return <HeroSection key={section.id} section={section} config={config} />;
      case 'products':
        return <ProductsSection key={section.id} section={section} config={config} produtos={[]} />;
      case 'content':
        return <ContentSection key={section.id} section={section} config={config} />;
      case 'contact':
        return <ContactSection key={section.id} section={section} config={config} />;
      default:
        return null;
    }
  };
      ...
      <div className="flex-1 overflow-hidden bg-gray-200 relative">
        <PreviewFrame viewMode={viewMode}>
          ...
          {headerSection ? (
            ...
            <div className="bg-gray-100 p-4 text-center text-sm text-gray-500 border-b">
              ⚠️ Header não configurado
            </div>
          )}
